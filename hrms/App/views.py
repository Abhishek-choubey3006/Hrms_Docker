...
def EmployPage(request):
    # log_activity(request.user, "Employee Visited dashboard")
    try:
    ...
    context = {
        'Employee': employee,
        'Notifications': Notifications.objects.filter().order_by('-created_at')[:5],
        'Meetings': Meeting.objects.filter(participants=employee).order_by('time_meeting')[:5],
...
def AdminPage(request):
    # today_birthdays, tomorrow_birthdays, upcoming_birthdays = birthday_helpers.get_birthdays()
    # today = timezone.localdate()
    # today_attendance = Attendance.objects.filter(date=today).select_related('user')
    # today = date.today()
    # today_attendance = Attendance.objects.filter(date=today)
    # on_time_attendance = [a for a in today_attendance if not a.is_late]
    # late_attendance = [a for a in today_attendance if a.is_late]
